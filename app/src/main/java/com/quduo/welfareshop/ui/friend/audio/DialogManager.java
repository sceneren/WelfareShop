...
public class DialogManager {
    ...
    public void showRecordingDialog() {
        ...
        int width = PtrLocalDisplay.SCREEN_WIDTH_PIXELS / 2;
        lp.width = width; // 宽度
        lp.height = width; // 高度
    ...
    public void wantToCancel() {
        ...
        if (mDialog != null && mDialog.isShowing()) {
            ...
            mIcon.setImageResource(R.drawable.ic_cancel_record_audio);
    ...
    public void updateVoiceLevel(int level) {
        ...
        if (mDialog != null && mDialog.isShowing()) {
            int resId;
            if (level >= 1 && level < 2) {
                resId = R.drawable.ic_voice_1;
            } else if (level >= 2 && level < 3) {
                resId = R.drawable.ic_voice_2;
            } else if (level >= 3 && level < 4) {
                resId = R.drawable.ic_voice_3;
            } else if (level >= 4 && level < 5) {
                resId = R.drawable.ic_voice_4;
            } else if (level >= 5 && level < 6) {
                resId = R.drawable.ic_voice_5;
            } else if (level >= 6 && level < 7) {
                resId = R.drawable.ic_voice_6;
            } else if (level >= 7 && level < 8) {
                resId = R.drawable.ic_voice_7;
            } else if (level >= 8 && level < 9) {
                resId = R.drawable.ic_voice_8;
            } else if (level >= 9 && level < 10) {
                resId = R.drawable.ic_voice_9;
            } else {
                resId = R.drawable.ic_voice_10;
            }
